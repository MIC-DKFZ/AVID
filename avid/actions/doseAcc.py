...
class DoseAccAction(CLIActionBase):
  ...
  def _indicateOutputs(self):
    ...
    if self._resultArtefact is None:
      ...
      self._batchArtefact = self.generateArtefact(self._dose2)
      ...
      for (pos, dose) in enumerate(doses):
        ...
        if len(linkedPlans) > 0:
          ...
          if planWeight is None:
            logger.warning("Selected plan has no fraction number information. Cannot determine fraction weight. Fall back to default strategy (weight: %s). Used plan artefact: %s", weight2, lPlan)
          else:
            weight2 = planWeight
          ...
          logger.info("No plan selected, no fraction number information available. Cannot determine fraction weight. Fall back to default strategy (1/number of selected doses => weight: %s).", weight2)
        ...
        if len(linkedRegs) > 0:
          lReg = linkedRegs[0]
          if len(linkedRegs) > 1:
            logger.warning("Improper selection of registrations. Multiple registrations for one dose/fraction selected. Action assumes only one registration linked per dose. Use first registration. Drop other registrations. Used registration: %s", lReg)
