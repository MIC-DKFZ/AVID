...
class TestKeyValueSorter(unittest.TestCase):
  def setUp(self):
    self.a1 = artefactGenerator.generateArtefactEntry("Case3", "3", 4, "Action1", "result", "dummy", None)
    self.a2 = artefactGenerator.generateArtefactEntry("Case5", "1", 1, "Action1", "result", "dummy", None)
    self.a3 = artefactGenerator.generateArtefactEntry("Case1", "20", 3, "Action2", "result", "dummy", None)
  ...
  def test_KeyValueSorter(self):
    ...
    self.assertEqual(self.a3, selection[2])
    self.assertEqual(self.a1, selection[3])
    self.assertEqual(self.a5, selection[4])

    sorter = TimePointSorter(reverse=True)
    selection = sorter.sortSelection(self.data)
    self.assertEqual(len(selection), 5)
    self.assertEqual(self.a5, selection[0])
