...
class LinkerBase(object):
  ...
  def _sanityCheck(self, primarySelection, linkedSelections):
    ...
    if self._allowOnlyFullLinkage:
      for linkedSelection in linkedSelections:
        if len(linkedSelection) == len(primarySelection) and not None in linkedSelection:
  ...
  def getLinkedSelection(self, primaryIndex, primarySelections, secondarySelections):
    ...
    return self._sanityCheck(primarySelection=primarySelection, linkedSelections=linkedSelections)
  ...
  def __add__(self,other):
    ...
    andLinker = AndLinker(self,other, allowOnlyFullLinkage=self._allowOnlyFullLinkage or other._allowOnlyFullLinkage)
    return andLinker
...
class AndLinker(LinkerBase):
  ...
  def _getLinkedSelection(self, primaryIndex, primarySelections, secondarySelections):
    '''Filters the given list of entries and returns all selected entries'''
    selections1 = self._linker1.getLinkedSelection(primaryIndex=primaryIndex, primarySelections=primarySelections,
                                                   secondarySelections=secondarySelections)
    selections2 = self._linker2.getLinkedSelection(primaryIndex=primaryIndex, primarySelections=primarySelections,
                                                   secondarySelections=secondarySelections)
    resultSelections = list(list(),)
