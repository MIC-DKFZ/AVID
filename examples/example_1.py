...
def my_function(outputs, **kwargs):
    ...
    for i, key in enumerate(keys):
        if i != 0:
            print_output += ", "
        print_output += "{}:{}".format(
            key, [os.path.basename(mr) for mr in kwargs[key]]
        )
    print_output += "}"
...
with workflow.initSession(
    ...
    action1 = PythonNaryBatchActionV2(
        primaryInputSelector=mr_image_selector,
    ...
    action2 = PythonNaryBatchActionV2(
        primaryInputSelector=action1.action_tag_selector,
